...
function fromRlp(value, to) {
    ...
    if (consumed < bytes.length)
        throw new index_js_1.DataLengthTooLongError({
            consumed,
            length: bytes.length,
        });
...
function parse(value) {
    if (typeof value === 'string') {
        if (value.length > 3 && value.length % 2 !== 0)
            throw new index_js_1.InvalidHexValueError(value);
...
function rlpToBytes(bytes, offset = 0) {
    ...
    if (prefix <= 0xb7) {
        ...
        if (offset_ + length > bytes.length)
            throw new index_js_1.DataLengthTooShortError({
    ...
    while (nextOffset < offset + consumed) {
        ...
        if (nextOffset > offset + consumed)
            throw new index_js_1.OffsetOutOfBoundsError({
