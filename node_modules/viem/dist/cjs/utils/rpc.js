...
async function http(url, { body, fetchOptions = {}, timeout = 10000 }) {
    ...
    try {
        const response = await (0, withTimeout_js_1.withTimeout)(async ({ signal }) => {
            ...
            errorInstance: new index_js_1.TimeoutError({ body, url }),
            timeout,
            signal: true,
        });
        let data;
        ...
        if (!response.ok) {
            throw new index_js_1.HttpRequestError({
                body,
        ...
        if (data.error) {
            throw new index_js_1.RpcRequestError({ body, error: data.error, url });
        }
        return data;
    }
    catch (err) {
        if (err instanceof index_js_1.HttpRequestError)
            throw err;
        if (err instanceof index_js_1.RpcRequestError)
            throw err;
        if (err instanceof index_js_1.TimeoutError)
            throw err;
        throw new index_js_1.HttpRequestError({
            body,
            details: err.message,
...
function webSocket(socket, { body, onData, onError, }) {
    ...
    const callback = ({ data }) => {
        ...
        if (typeof message.id === 'number' && id_ !== message.id)
            return;
        if (message.error) {
            onError?.(new index_js_1.RpcRequestError({ body, error: message.error, url: socket.url }));
        }
        else {
            onData?.(message);
        }
...
async function webSocketAsync(socket, { body, timeout = 10000, }) {
    return (0, withTimeout_js_1.withTimeout)(() => new Promise((onData, onError) => exports.rpc.webSocket(socket, {
        ...
        errorInstance: new index_js_1.TimeoutError({ body, url: socket.url }),
        timeout,
    });
}
exports.rpc = {
