...
async function waitForTransactionReceipt(client, { confirmations = 1, hash, onReplaced, pollingInterval = client.pollingInterval, timeout, }) {
    ...
    return new Promise((resolve, reject) => {
        if (timeout)
            setTimeout(() => reject(new index_js_1.WaitForTransactionReceiptTimeoutError({ hash })), timeout);
        const _unobserve = (0, observe_js_1.observe)(observerId, { onReplaced, resolve, reject }, (emit) => {
            const unwatch = (0, index_js_2.watchBlockNumber)(client, {
                emitMissed: true,
                emitOnBegin: true,
                poll: true,
                pollingInterval,
                async onBlockNumber(blockNumber) {
                    ...
                    try {
                        ...
                        if (transaction &&
                            (err instanceof index_js_1.TransactionNotFoundError ||
                                err instanceof index_js_1.TransactionReceiptNotFoundError)) {
                            replacedTransaction = transaction;
                            const block = await (0, index_js_2.getBlock)(client, {
                                blockNumber,
